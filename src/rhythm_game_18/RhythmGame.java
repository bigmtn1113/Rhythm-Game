...
public class RhythmGame extends JFrame {
	...
	private Image scoreImage = new ImageIcon(Main.class.getResource("../images/Game Score Image.png")).getImage();
	...
	public static Game game;	// 게임은 하나만 실행되어야 함. 프로젝트 중 오직 하나만 존재해야 함
	
	public RhythmGame() {
		// trackList.add 하기 전에 버튼 이벤트가 발생하여 음악을 실행하게 되면 오류가 남. 그래서 trackList.add 먼저 실행
		trackList.add(new Track("Shadows Title Image.png", "Shadows Start Image.png",
				"Game Play Image.png", "3rd Prototype - Shadows.mp3", "3rd Prototype - Shadows"));
		trackList.add(new Track("Rich Title Image.png", "Rich Start Image.png",
				"Game Play Image.png", "Raven & Kreyn - RICH.mp3", "Raven & Kreyn - RICH"));
		trackList.add(new Track("Animal Title Image.png", "Animal Start Image.png",
				"Game Play Image.png", "Debris & RudeLies - Animal (feat. Jex).mp3", "Debris & RudeLies - Animal (feat. Jex)"));
	...
	public void screenDraw(Graphics2D g) {	// 글씨 깨짐 없애기
		...
		if (isGameFinishScreen) {
			g.drawImage(scoreImage, 340, 150, null);
			g.setColor(Color.WHITE);
			g.setFont(new Font("Elephant", Font.BOLD, 40));
			g.drawString(game.getEarlyCount().toString(), 830, 205);
			g.drawString(game.getGoodCount().toString(), 830, 270);
			g.drawString(game.getGreatCount().toString(), 830, 330);
			g.drawString(game.getPerfectCount().toString(), 830, 390);
			g.drawString(game.getLateCount().toString(), 830, 450);
			g.drawString(game.getMissCount().toString(), 830, 510);
